			...
			for (const [name, fields] of Object.entries(named_fields)) {
				if (fields.length > 1) { // 推断数组类型
					...
					if (type === DataType.null) {
						console.log(colors.red(`\t\t${name}(${xlsl.utils.encode_col(fields[0].columns.start)}列) 没有填入有效数据, 无法正确推断数据类型`));
					}
				}
			}
			for (const c of this.children) {
				...
				if (c.type === DataType.null && !c._is_array) {
					console.log(colors.red(`\t\t${c.name}(${xlsl.utils.encode_col(c.columns.start)}列) 没有填入有效数据, 无法正确推断数据类型`));
				}
			}
		}
	}
